...
func (s *Server) registerServerTools() {
	...
	// Get all supported tool types from the factory
	supportedTools := factory.GetSupportedToolTypes()

	for _, toolType := range supportedTools {
		// Create tool instance using factory with minimal dependencies (only AWS client)
	...
	for propName, propDef := range properties {
		...
		case "array":
			...
			if hasItems {
				...
				switch itemType {
				case "string":
					arrayOptions := append(paramOptions, mcp.WithStringItems())
					options = append(options, mcp.WithArray(propName, arrayOptions...))
				case "number", "integer":
					arrayOptions := append(paramOptions, mcp.WithNumberItems())
					options = append(options, mcp.WithArray(propName, arrayOptions...))
				case "boolean":
					arrayOptions := append(paramOptions, mcp.WithBooleanItems())
					options = append(options, mcp.WithArray(propName, arrayOptions...))
				default:
